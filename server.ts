...
import { publishHtmlToGitHub } from "./githubPublisher";

dotenv.config();

// ============================================================
// renderCvHtml
// Pure function: takes a CV object + language, returns the full
// HTML string for the hosted CV page. Used by BOTH the live
// "/cv/:username" route AND the "/api/publish-cv/:username"
// GitHub Pages publisher, so both always produce identical output.
// ============================================================
function renderCvHtml(cv: any, activeLang: "en" | "ar"): string {
...
async function startServer() {
    ...
    if (isDbConnected()) {
      try {
        ...
        const cvToSave = {
          ...
          experience: mergedCvData.experience || "",
  ...
  // API: Publish the CV as a static page to GitHub Pages
  app.post("/api/publish-cv/:username", async (req, res) => {
    ...
    if (isDbConnected()) {
      try {
        ...
        console.error("Failed to load CV from MongoDB for publishing:", err);
      }
    }
    if (!cv) {
      const cvs = loadCVs();
    ...
    const lang: "en" | "ar" = cv.language === "ar" ? "ar" : "en";
    const html = renderCvHtml(cv, lang);

    const result = await publishHtmlToGitHub(username, html);

    if (!result.success) {
      return res.status(500).json({ error: result.error });
    }

    // Persist the published URL/state back to MongoDB, if connected
    if (isDbConnected()) {
      try {
        await CV.updateOne(
          { username },
          { githubPagesUrl: result.url, isPublishedToGithub: true }
        );
      } catch (err) {
        console.error("Failed to update publish status in MongoDB:", err);
      }
    }
...
${JSON.stringify({
  ...
  app.get("/cv/:username", async (req, res) => {
    ...
    const activeLang: "en" | "ar" = (queryLang === "ar" || queryLang === "en") ? queryLang : (cv.language === "ar" ? "ar" : "en");

    res.send(renderCvHtml(cv, activeLang));
  });
